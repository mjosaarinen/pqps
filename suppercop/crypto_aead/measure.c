...
// XXX mjos 
        if (direction != 1) mlen += 1 + mlen / 3;
        if (direction != 0) adlen += 1 + adlen / 3;
...
//		if (mlen > 64) if (mlen & 31) if (adlen != mlen) continue;
 //       if (adlen > 64) if (adlen & 31) if (adlen != mlen) continue;
		...
		printentry(1000000 * adlen + mlen, "aead_enc_njoule", &nj, 1);
		printentry(1000000 * adlen + mlen, "aead_enc_avgcyc", &cyc, 1);
...
//        if (adlen > 64) if (adlen & 31) continue;
       ...
       for (i = 0;i <= TIMINGS;++i) {
		...
		printentry(1000000 * adlen + mlen, "aead_dec_njoule", &nj, 1);
		printentry(1000000 * adlen + mlen, "aead_dec_avgcyc", &cyc, 1);
		...
		ENERGY_END
          ...
          printentry(1000000 * adlen + mlen,"forgery_decrypt_cycles",cycles,TIMINGS);
			...
			printentry(1000000 * adlen + mlen, "aead_decf_njoule", &nj, 1);
			printentry(1000000 * adlen + mlen, "aead_decf_avgcyc", &cyc, 1);
